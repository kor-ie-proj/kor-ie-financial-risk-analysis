...
export async function fetchFinancials(corpName: string) {
    return getJson<FinancialResponse>(
        `/companies/${encodeURIComponent(corpName)}/financials`
    );
}

export async function fetchRisk(corpName: string, monthsToPredict = 3) {
    return getJson<RiskResponse>(
        `/companies/${encodeURIComponent(corpName)}/risk`,
        {
...
export async function fetchManualRisk(
    corpName: string,
    payload: ManualRiskRequest
) {
